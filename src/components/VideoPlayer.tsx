...
const VideoPlayer: React.FC = memo(() => {
  ...
  const config = useVideoStore(state => state.config)
  const setConfig = useVideoStore(state => state.setConfig)
  ...
  useEffect(() => {
        ...
        if (video.duration > 0 && video.currentTime >= (video.duration - (config.skipIntro ? 36 : 1))) {
          switch (config.mode) {
            ...
            case 'random':
              ...
              break;
            case 'loop':
              video.currentTime = (config.skipIntro ? 10 : 1)
              break;
          }
        }
      }
    ...
    // 改变网站title
    videoInfo && (document.title = '宁路 | ' + videoInfo?.title)
    return () => {
  ...
  const speedOnChange = (e: SelectChangeEvent) => {
    ...
    setConfig({ speed: +e.target.value })
  }
    ...
    {videoInfo?.no &&
      ...
      >
        <video
          ...
          ref={videoRef}
          src={`${import.meta.env.VITE_STREAM_URL}?code=${videoInfo.no}&format=${(config.quality === 'mp3' || isbackground) ? 'mp3' : 'mp4&width=' + config.quality}&${sessionStorage.getItem("date_auth")}`}
        >
          您的浏览器不支持 HTML5 视频。
        </video>
        <Box sx={{
          ...
          <FormControlLabel
            control={<Switch size='small' checked={!config.skipIntro}
              onChange={() => setConfig({ skipIntro: !config.skipIntro })} />}
            label="片头"
          />
          ...
          <SmallFormControl label='画质' selectedValue={config.quality ?? ''} onChange={(e: SelectChangeEvent) => {
            setConfig({ quality: e.target.value })
          }} options={[
            { name: '仅声音', value: 'mp3' },
            { name: '标清', value: '480' },
            { name: '高清', value: '720' },
